...
EquipmentSelector::EquipmentSelector(QWidget *parent) :QWidget(parent)
{
  //lbl_name=new QLabel();
    //lbl_icon=new QLabel();
    //lbl_icon->setFixedSize(24,24);
    //lbl_icon->setScaledContents(1);
    combo_selector = new QComboBox;
    lbl_desc=new QLabel();
  ...
  this->adjustSize();
  connect(combo_selector,SIGNAL(currentIndexChanged(int)),this,SLOT(ItemChange(int)));
}
void EquipmentSelector::ItemChange(int index){setItem(index + min);}

void EquipmentSelector::setMode(int mode)
{//set EquipmentMode
    if(mode ==0){setRange(0,1);}
    else if(mode ==1){setRange(128,144);}
    else if(mode ==2){setRange(160,176);}
    else if(mode ==3){setRange(144,160);}
    else if(mode ==4){setRange(190,201);}
    else if(mode ==5){setRange(176,190);}
    else if(mode ==6){setRange(215,229);}
    else if(mode ==7){setRange(229,242);}
    else if(mode ==8){setRange(242,255);}
    else if(mode ==9){setRange(201,215);}
    else if(mode ==10){setRange(128,144);}
    else if(mode ==11){setRange(255,256);}

    else if(mode ==12){setRange(256,288);}
    else if(mode ==13){setRange(288,320);}
    else{return;}
}
void EquipmentSelector::setRange(int start,int end)
{
    min = start;
    max = end;
    combo_selector->clear();
    for(int i=min;i<max;i++){combo_selector->addItem(QPixmap::fromImage(data.Image(i)),data.Name(i));}
    combo_selector->adjustSize();
}
void EquipmentSelector::setDesc(QString text){lbl_desc->setText(text);lbl_desc->adjustSize();}

void EquipmentSelector::setItem(int id)
{
    lbl_slot_8->setHidden(1);
    lbl_slot_7->setHidden(1);
    lbl_slot_6->setHidden(1);
...
void EquipmentSelector::elemental_info(int id)
{
    int y=20;
    bool show=false;
...
void EquipmentSelector::status_info(int id)
{
    int y=20;
    bool show=false;
