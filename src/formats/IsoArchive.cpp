...
{
    return _size / SECTOR_SIZE_DATA + uint(_size % SECTOR_SIZE_DATA != 0);
}
...
{
    return _newSize / SECTOR_SIZE_DATA + uint(_newSize % SECTOR_SIZE_DATA != 0);
}
...
IsoFile::IsoFile(const QString &name, quint32 location, quint32 size, qint64 structPosition, IsoArchiveIO *io) :
    IsoFileOrDirectory(name, location, size, structPosition),
    _io(nullptr), _newIO(nullptr), dataChanged(false),
...
{
    maxSize = maxSize == 0 ? size() : std::min(maxSize, size());
    ...
    while (maxSize - totalR > 0 && (r = _io->read(data, std::min(qint64(MAX_ISO_READ), maxSize - totalR))) > 0) {
        ret.write(data, r);
        totalR += r;
    }
...
{
    char *data = new char[quint64(maxSize)];
...
QByteArray IsoArchiveIO::sector(quint32 num, quint16 maxSize)
{
    ...
    return read(std::min(maxSize, quint16(SECTOR_SIZE_DATA)));
}

quint32 IsoArchiveIO::currentSector() const
{
    return quint32(pos() / SECTOR_SIZE);
}

quint32 IsoArchiveIO::sectorCount() const
{
    return quint32(size() / SECTOR_SIZE);
}
...
qint64 IsoFileIO::readData(char *data, qint64 maxSize)
{
    if (_io->seekToSector(_infos->location()) && _io->seekIso(_io->posIso() + pos())) {
        ...
        return _io->readIso(data, std::min(maxSize, size - pos()));
    }
    return -1;
}
...
{
    int i = 0;
    for (IsoFileOrDirectory *fileOrDir : filesWithPadding) {
...
//#endif
            ...
            if (index >= 0) {
                ...
                isoFile->setPaddingAfter(quint8(fileWithPaddingAfter->paddingAfter() - isoFile->newSectorCount()));
...
//#endif
            ...
            if (isoFile->paddingAfter() > isoFile->newSectorCount() - isoFile->sectorCount()) {
                isoFile->setPaddingAfter(quint8(isoFile->paddingAfter() - (isoFile->newSectorCount() - isoFile->sectorCount())));
            }
            writeToTheMain.insert(isoFile->newLocation(), isoFile);
        }
    }
...
#ifdef ISOARCHIVE_DEBUG
        if (destinationIO->pos() % SECTOR_SIZE != 0) {
            qWarning() << "destination error 1a" << (destinationIO->pos() % SECTOR_SIZE) << isoFile->name();
        }
        if (destinationIO->currentSector() != fileLocation) {
            qWarning() << "destination error 1b" << destinationIO->currentSector() << fileLocation << isoFile->name();
        }
        if (destinationIO->pos() != _io.pos()) {
            qWarning() << "destination error 1c" << destinationIO->pos() << _io.pos() << isoFile->name();
        }
#endif
...
#ifdef ISOARCHIVE_DEBUG
        if (destinationIO->pos() % SECTOR_SIZE != 0) {
            qWarning() << "destination error 2a" << (destinationIO->pos() % SECTOR_SIZE) << isoFile->name();
        }
        if (destinationIO->currentSector() != _io.currentSector()) {
            qWarning() << "destination error 2b" << destinationIO->currentSector() << _io.currentSector() << isoFile->name();
        }
        if (destinationIO->pos() != _io.pos()) {
            qWarning() << "destination error 2c" << destinationIO->pos() << _io.pos() << isoFile->name();
        }
#endif

        // Envoi de la position courante à l'output
        if (control) {
            control->setObserverValue(int(destinationIO->currentSector()));
        }
    }
...
#ifdef ISOARCHIVE_DEBUG
    if (destinationIO->pos() % SECTOR_SIZE != 0) {
        qWarning() << "destination error 3a" << (destinationIO->pos() % SECTOR_SIZE);
    }
    if (destinationIO->currentSector() != endOfFilledIso) {
        qWarning() << "destination error 3b" << destinationIO->currentSector() << _io.sectorCount();
    }
    if (destinationIO->pos() != _io.pos()) {
        qWarning() << "destination error 3c" << destinationIO->pos() << _io.pos();
    }
#endif
...
#ifdef ISOARCHIVE_DEBUG
        if (destinationIO->pos() % SECTOR_SIZE != 0) {
            qWarning() << "destination error 3a" << (destinationIO->pos() % SECTOR_SIZE) << isoFile->name();
        }
#endif
...
#ifdef ISOARCHIVE_DEBUG
    if (destinationIO->size() % SECTOR_SIZE != 0) {
        qWarning() << "Invalid size" << destinationIO->size();
    }
#endif
...
bool IsoArchive::copySectors(IsoArchiveIO *out, qint64 sectorCount, ArchiveObserver *control, bool repair)
{
    ...
    for (int i = 0; i < sectorCount; ++i) {
        ...
        // Send current position to output
        if (control) {
...
IsoDirectory *IsoArchive::_openDirectoryRecord(IsoDirectory *directories, QList<quint32> &dirVisisted)
{
    ...
    // anti-loop forever
    if (dirVisisted.contains(sector)) {
        return directories;
    }
    ...
    {
        const qint64 maxPos = _io.posIso() + dataSize;
...
qint32 IsoArchive::diffCountSectors(const QString &path, quint32 newSize) const
{
    ...
    return qint32(_io.sectorCountData(newSize - isoFile->size()));
}
...
{
    if (prevFile != nullptr) {
        prevFile->setPaddingAfter(quint8(fileLocation - prevFile->locationAfter()));
        ...
        if (prevFile->paddingAfter() > 0) {
            ...
            prevFile->setPaddingAfter(quint8(sector));
