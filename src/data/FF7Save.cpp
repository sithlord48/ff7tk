...
QByteArray FF7Save::slotPsxRawData(int s)
{
    if (filename.isEmpty()) {
        return QByteArray();
    } else if (FF7SaveInfo::isTypePC(fileFormat)) {
        return QByteArray();
    } else if (FF7SaveInfo::isTypeSSS(fileFormat)) {
        QFile file(fileName());
        if (!file.open(QIODevice::ReadOnly))
...
void FF7Save::newGamePlus(int s, QString CharFileName, QString fileName)
{
    ...
    std::copy(std::begin(slot[s].items), std::end(slot[s].items), std::begin(tempSlot.items));
    ...
    std::copy ( std::begin(slot[s].chocostaminas), std::end(slot[s].chocostaminas), std::begin(tempSlot.chocostaminas));
