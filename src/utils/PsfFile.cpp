...
bool PsfTags::open(const QString &config)
{
    ...
    if (!config.startsWith(QStringLiteral("[TAG]"))) {
        return false;
    }
    ...
    for (const QString &entry : qAsConst(entries)) {
        ...
        if (index >= 0) {
            ...
            QString value = entry.mid(index + 1);
...
{
    return setCustom(QStringLiteral("length"), QStringLiteral("%1:%2").arg(seconds / 60).arg(seconds % 60));
}

PsfTags &PsfTags::setReplayGain(const QString &type, double gain_db, double peak)
{
    return setCustom(QStringLiteral("replaygain_%1_gain").arg(type), QStringLiteral("%1%2")
                     .arg(gain_db < 0 ? QLatin1String("-") : QLatin1String("+"))
                     .arg(gain_db))
            .setCustom(QStringLiteral("replaygain_%1_peak").arg(type), QString::number(peak));
}
